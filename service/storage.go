...
// UploadAssessmentThumbnail validates and uploads an assessment thumbnail to Supabase Storage.
// Files are stored under the assessments/thumbnails/ prefix. Returns the public URL.
func (s *StorageService) UploadAssessmentThumbnail(fh *multipart.FileHeader) (string, error) {
	...
	filename := fmt.Sprintf("assessments/thumbnails/%s%s", randomHex(), ext)
	uploadURL := fmt.Sprintf("%s/storage/v1/object/%s/%s",
		strings.TrimRight(s.cfg.ProjectURL, "/"),
		s.cfg.Bucket,
		filename,
...
// UploadAssessmentFile validates and uploads an assessment file (PDF, doc, etc.) to Supabase Storage.
// Files are stored under assessments/files/ in the materials bucket. Returns the public URL.
func (s *StorageService) UploadAssessmentFile(fh *multipart.FileHeader) (string, error) {
	...
	filename := fmt.Sprintf("assessments/files/%s%s", randomHex(), meta.ext)
	uploadURL := fmt.Sprintf("%s/storage/v1/object/%s/%s",
		strings.TrimRight(s.cfg.ProjectURL, "/"),
		s.cfg.MaterialBucket,
		filename,
...
// UploadAssignmentFile validates and uploads a student's assignment submission file
// to Supabase Storage. Files are stored under assignments/submissions/ in the
// materials bucket. Returns the public URL.
func (s *StorageService) UploadAssignmentFile(fh *multipart.FileHeader) (string, error) {
	...
	filename := fmt.Sprintf("assignments/submissions/%s%s", randomHex(), meta.ext)
	uploadURL := fmt.Sprintf("%s/storage/v1/object/%s/%s",
		strings.TrimRight(s.cfg.ProjectURL, "/"),
		s.cfg.MaterialBucket,
		filename,
...
// UploadResourceFile validates and uploads a learning resource file (PDF, doc,
// video, image, code, etc.) to Supabase Storage. Files are stored under
// resources/files/ in the materials bucket. Returns the public URL.
func (s *StorageService) UploadResourceFile(fh *multipart.FileHeader) (string, error) {
	...
	filename := fmt.Sprintf("resources/files/%s%s", randomHex(), meta.ext)
	uploadURL := fmt.Sprintf("%s/storage/v1/object/%s/%s",
		strings.TrimRight(s.cfg.ProjectURL, "/"),
		s.cfg.MaterialBucket,
		filename,
...
// UploadProjectFile validates and uploads a project submission file (report,
// zip, presentation, etc.) to Supabase Storage. Files are stored under
// projects/submissions/ in the materials bucket. Returns the public URL.
func (s *StorageService) UploadProjectFile(fh *multipart.FileHeader) (string, error) {
	...
	filename := fmt.Sprintf("projects/submissions/%s%s", randomHex(), meta.ext)
	uploadURL := fmt.Sprintf("%s/storage/v1/object/%s/%s",
		strings.TrimRight(s.cfg.ProjectURL, "/"),
		s.cfg.MaterialBucket,
		filename,
