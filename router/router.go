...
func New(pool *pgxpool.Pool, cfg *config.Config) *gin.Engine {
	...
	sessionCtrl := controller.NewSessionController(sessionRepo, batchRepo, notificationRepo, userRepo, zoomSvc, emailSvc, storageSvc, cfg.App.PublicURL, cfg.App.Timezone, cfg.App.StudentPortalURL, cfg.App.AdminPortalURL, auditLogRepo, feedbackFormRepo, batchRecordingRepo)
	...
	v1 := r.Group("/api/v1")
	{
		...
		protected.Use(middleware.JWTAuth(cfg.JWT.Secret))
		{
			...
			sessions := protected.Group("/sessions", middleware.RequireActiveSubscription(collegeRepo), middleware.RequireFeature(collegeRepo, models.FeatureLiveSessions))
			{
				...
				sessions.GET("/:short_id/join", sessionCtrl.JoinSession)
